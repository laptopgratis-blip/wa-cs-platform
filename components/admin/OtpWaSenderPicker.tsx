...
export function OtpWaSenderPicker() {
  ...
  async function load(): Promise<ConnectedSession[]> {
    ...
    try {
      ...
      const list =
        sessionsJson.success && sessionsJson.data ? sessionsJson.data : []
      setSessions(list)
  ...
  async function handleSessionConnected() {
    toast.success(
      'Nomor terhubung — pilih lalu Simpan untuk aktif sebagai OTP sender',
    )
  ...
  useEffect(() => {
    ...
    const t = window.setTimeout(() => {
      fetch(
        `/api/admin/platform-templates?sessionId=${encodeURIComponent(currentValue)}`,
      )
        .then(
          (r) =>
            r.json() as Promise<{
        ...
        .then((json) => {
          if (!cancelled)
            setPlatformItems(json.success && json.data ? json.data.items : [])
        })
        .catch(() => {
    ...
    <Card className="border-warm-200 rounded-xl">
      <CardHeader className="pb-2">
        ...
        <CardDescription className="text-warm-500 text-xs">
          ...
          Halaman login/register user otomatis menyesuaikan pilihan channel di
          sini.
        </CardDescription>
      </CardHeader>
        ...
        {loading ? (
          ...
          <>
            <fieldset className="space-y-2">
              <legend className="text-warm-500 mb-1 text-xs font-semibold tracking-wider uppercase">
                Channel pengiriman
              </legend>
              {CHANNEL_OPTIONS.map((opt) => (
                ...
                >
                  ...
                  <div className="flex-1">
                    <div className="text-warm-900 text-sm font-semibold">
                      {opt.label}
                    </div>
            ...
            >
              ...
              <fieldset className="space-y-2">
                ...
                {sessions.length === 0 ? (
                  <div className="border-warm-300 bg-warm-50/40 text-warm-500 rounded-lg border border-dashed p-4 text-center text-xs">
                  ...
                  sessions.map((s) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        />
                        <div className="flex-1">
                          <div className="text-warm-900 flex items-center gap-2 text-sm font-semibold">
