...
import { ArrowLeft, ChevronLeft, ChevronRight, Eye, Flame, Package, ShoppingBag, X, Zap } from 'lucide-react'
        ...
        <div className="overflow-y-auto pb-4">
          ...
          {gallery.length > 0 ? (
            <div className="relative mx-3 overflow-hidden rounded-2xl bg-warm-100" style={{ aspectRatio: '1' }}>
              ...
              {flashOn ? (
                <div className="absolute left-2 top-2 flex items-center gap-0.5 rounded-md bg-gradient-to-r from-red-600 to-orange-500 px-2 py-1 text-xs font-black uppercase tracking-wider text-white shadow-lg">
                  <Zap className="size-3.5" aria-hidden="true" /> -{discountPct}%
                </div>
              ) : null}
            </div>
          ) : null}
          ...
          <div className="px-4 pt-4">
            ...
            {flashQuotaRemaining != null && flashQuotaRemaining > 0 ? (
              <div className="mt-1.5 flex items-center gap-1 text-[11px] font-semibold text-red-700">
                <Flame className="size-3.5" aria-hidden="true" /> Tinggal {flashQuotaRemaining} dari batch flash sale
              </div>
            ) : null}
          </div>
          ...
          {outOfStock ? (
            ...
            <div className="mx-4 mt-3 flex items-center gap-1 rounded-lg bg-amber-50 px-3 py-2 text-xs font-semibold text-amber-800">
              <Flame className="size-3.5" aria-hidden="true" /> Tinggal {effectiveStock} stok!
            </div>
          ) : null}
