...
export function ScoreHistoryChart({ lpId }: Props) {
  ...
  const chart = useMemo(() => {
    ...
    const maxTs = new Date(
      data.points[data.points.length - 1]!.computedAt,
    ).getTime()
    ...
    const yOf = (score: number) => H - PAD_Y - (score / 100) * (H - PAD_Y * 2)

    const path = data.points
      .map(
        (p, i) =>
          `${i === 0 ? 'M' : 'L'} ${xOf(p.computedAt).toFixed(1)} ${yOf(p.total).toFixed(1)}`,
      )
      .join(' ')
    ...
    <Card>
      <CardContent>
        ...
        {!loading && (!data || data.points.length === 0) && (
          <div className="border-warm-200 bg-warm-50 text-warm-500 rounded border border-dashed py-8 text-center text-xs">
        ...
        {!loading && data && data.points.length > 0 && chart && (
          <>
            ...
            >
              ...
              {data.points.map((p, i) => (
                ...
                >
                  <title>
                    {p.total}/100 ·{' '}
                    {new Date(p.computedAt).toLocaleString('id-ID')} · trigger:
                    {p.trigger} · {p.sampleVisits} visit
                  </title>
                </circle>
              ))}
            ...
            <div className="text-warm-500 mt-2 flex items-center gap-3 text-xs">
              ...
              {data.applyMarkers.length > 0 && (
                <span className="flex items-center gap-1">
                  <span className="bg-warm-500 size-2 rounded-full" />
